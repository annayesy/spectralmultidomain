...
class LeafSubdomainJAX:
    def __init__(self, box_centers, pdo, patch_utils):
        ...
        self.xxloc_int  = patch_utils.zz_int[None, :, :] + centers[:, None, :]
        self.xxloc_ext  = patch_utils.zz_ext[None, :, :] + centers[:, None, :]
        ...
        self.Nx_cheb     = patch_utils.Nx_stack
        self.chebfleg_mat  = patch_utils.chebfleg_exterior_mat
        self.legfcheb_mat  = patch_utils.legfcheb_exterior_mat

    def Aloc_helper(self, start, end):
        # returns (batch_chunk, nt, nt)
        return get_Aloc(self.pdo,
                        self.xxloc_int[start:end],
                        self.diff_ops)
    ...
    def solve_dir_helper(self, start, end, uu_dir=None, ff_body=None):
        ...
        _,inds_uni = jnp.unique(self.Jx_cheb,return_index=True)

        res = res.at[:, self.Jx_cheb_uni].set(tmp[...,inds_uni,:])
    ...
    def solve_dir(self, uu_dir, ff_body=None):
        ...
        res = jnp.zeros((self.nbatch, self.nt_cheb, uu_dir.shape[-1]))

        for start in range(0, self.nbatch, 10):
            end = min(start + 10, self.nbatch)
            res = res.at[start:end].set(
                self.solve_dir_helper(start, end, uu_dir, ff_body)
            )
        return res
    ...
    def reduce_body_load(self, ff_body):
        ...
        loc_sol = self.solve_dir(jnp.zeros((self.nbatch, self.nx_leg, 1)), -ff_body)
        tmp     = self.Nx_cheb[:, self.Ji_cheb] @ loc_sol[:, self.Ji_cheb]
        return self.legfcheb_mat @ tmp

    @property
    def DtN(self):
        DtNs = jnp.zeros((self.nbatch, self.nx_leg, self.nx_leg))
        for start in range(0, self.nbatch, 10):
            end = min(start + 10, self.nbatch)
            loc = self.solve_dir_helper(start, end)
